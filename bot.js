...
var jaky = ["úchylný/á","hloupý/á","zamilovaný/á","retardovaný/á","pošahaný/á","chytrý/á","mocný/á","sexy","nemocný/á","vožralý/á"];
...
var proc = ["protože jsou prostě retardi.","protože bagr","protože je nikdo nemá rád.","protože jsou do sebe zamilovaní.","protože jsou zhulený.","protože chcou více drog.","protože jsou vožralý.","protože banán.","protože když máš 4 jablka a 3 hrušky, nemůžeš mít švestkový kompot."];
...
API.sendChat("[@"+ from +"] Jednoho večera zazvonil/a u tebe tvůj/tvoje " + jaky[Math.floor(Math.random() * jaky.length)] + " kamarád/ka " + uzivatel[(cislo - 1)].username + " s tím, ať jdete " + kam[Math.floor(Math.random() * kam.length)] + " Odpověděl/a jsi, že ne, radši půjdeš " + co[Math.floor(Math.random() * co.length)] + ". Nakonec tě přemluvil/a.");
setTimeout(function(){ API.sendChat("Cestou jste potkali " + jake[Math.floor(Math.random() * jake.length)] + " kamarády " + uzivatel[(cislo - 1)].username + " a " + uzivatel[(cislo - 1)].username + ". Zeptal/a ses, jestli chtějí jít s vámi. Na to ti " + jak[Math.floor(Math.random() * jak.length)] + " odpověděli, ať na to zapomeneš " + proc[Math.floor(Math.random() * proc.length)] + ""); }, 1000);
